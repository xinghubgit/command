...
--restart=always 192.168.250.121:6000/csf/svc/csf-analysis-svc:v0.21.5-mysql   -e local -p 8081  -s false -m true
...
cat  nlpsvc.log  | grep 'finish analysis news'  | awk -F' ' '{print $1 " " $9}' | sed 's/\.[0-9][0-9][0-9]//g' |  sed 's/news\[\(.*\)\]/\1/g' | awk -F' ' '{s[$1] += 1}END {for (i in s) {print i, s[i]}}' | awk -F' ' '{sum += $2}END{print "Average=", sum/NR}'
